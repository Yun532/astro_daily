...
class ClaudePaperAnalyst:
    ...
    def _expand_weekend_lesson_sections(self, lesson: WeekendLesson, *, run_date: date) -> WeekendLesson:
        ...
        for fields in section_groups:
            try:
                updates = self._expand_weekend_lesson_section_group(expanded, run_date=run_date, fields=fields)
    ...
    def _expand_weekend_lesson_section_group(self, lesson: WeekendLesson, *, run_date: date, fields: list[str]) -> dict[str, str]:
        payload = {
            "date": run_date.isoformat(),
            ...
            "instruction": (
                "Rewrite only requested_fields. Preserve the chosen course scope and make these fields systematic and course-like. "
